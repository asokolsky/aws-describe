...
def find_instances_by_asg(service:str, region:str, acct_id:str, res_type:str,
        res_name:str) -> List[Dict[str, Any]]:
    '''
    '''
...
def find_instances_by_filter(id:str, filters: List[Dict[str, Any]]) -> List[
        Dict[str, Any]]:
    '''
...
def find_instances(id:str, verbose:bool, format:common.OutputOption) -> Tuple[
        int, List[str]]:
    '''
    ...
    if filters:
        ...
        for p,msg,foo in dispatch_table:
            if p.search(id):
                if verbose:
                    print(f'Treating {id} as {msg}', file=sys.stderr)
                prefix,aws,service,region,acct_id,res_type,res_name = common.parse_arn(
                    id)
    ...
    if verbose:
        ...
        total = len(instances)
        print(f"Found {total} instance{'s'[:total^1]}:",
                file=sys.stderr)
    ...
    if format == common.OutputOption.ids:
        ...
        for i in instances:
            print(i['InstanceId'], common.get_tag(i, 'Name'))
    else:
        print(json.dumps(
            instances, indent=4, default=common.json_datetime_serializer))
...
def terminate_instances(ids: List[str], verbose:bool) -> int:
    ...
    if verbose:
        for i in res:
            print(f"{i['InstanceId']}: from '{i['PreviousState']['Name']}' to '{i['CurrentState']['Name']}'")
    return 0
